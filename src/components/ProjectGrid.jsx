...
function ProjectDialog({ project, lang, copy, onClose }) {
  ...
  uniqueImages.forEach((imgUrl, idx) => {
    mediaItems.push({
      type: "image",
      url: imgUrl,
      alt: project.media.alt?.[lang] || project.media.alt?.en || `${localized.title} - ${idx + 1}`,
      label: `${copy.projectImage} ${idx + 1}`,
    });
  });

  if (project.media.video?.url) {
    mediaItems.push({
      type: "video",
  ...
  return (
    <div className="project-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        {/* Media Container (Unified for Images and Video) */}
        <div className="project-dialog-media">
          {activeMedia?.type === "video" ? (
            <video
              ...
              className="project-dialog-video-player"
            >
              <source src={activeMedia.url} type={activeMedia.videoType} />
            ...
            <img
              key={activeMedia?.url}
              src={activeMedia?.url}
              alt={activeMedia?.alt || localized.title}
        ...
        <div className="project-dialog-content">
          <div className="project-dialog-heading">
            <div>
              <p className="project-dialog-kicker">{copy.projectDetails}</p>
              <h2 id={`project-${project.id}-title`}>{localized.title}</h2>
            </div>
          ...
          {/* Unified Media Gallery / Selector */}
          {mediaItems.length > 1 && (
            <div className="project-gallery" aria-label={copy.projectImages}>
              {mediaItems.map((item, index) => {
                const isActive =
                  activeMedia?.type === item.type && activeMedia?.url === item.url;
                return (
                  <button
                    type="button"
                    key={`${item.type}-${item.url || index}`}
                    className={`project-gallery-thumb ${isActive ? "is-active" : ""}`}
                    onClick={() => setActiveMedia(item)}
                    aria-label={item.label}
                  >
                    {item.type === "video" ? (
                      <div className="project-gallery-video-thumb">
                        {item.poster ? (
                          <img src={item.poster} alt="Video thumbnail" />
                        ) : (
                          <div className="project-gallery-video-placeholder" />
                        )}
                        <span className="project-gallery-play-icon">
...
export function ProjectGrid() {
  ...
  const sectionCopy = projectsData[lang];
              ...
              return (
                ...
                >
                  <button
                    ...
                    aria-label={`${sectionCopy.openProject}: ${localized.title}`}
                  >
                    ...
                    <div className="project-card-content">
                      <h3 className="text-xl font-bold text-[var(--text-projects-title)]">
                        {localized.title}
                      </h3>
                      <p
